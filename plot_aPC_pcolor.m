% code to plot PSTH for aPC, trial-avg frates in pcolor plot
...
nmND=165; nmBic=195; nmMus=39; %size(PSTH_Orth_Mus,3);
%% 
...
% get scaled colormap
x = 1:512;
x = x-(.98*mxFR-mnFR)*512/(mxFR-mnFR); %centerPoint=0.9*mxFR
x = .2*x/max(abs(x));  %scaleIntensity=5
x = sign(x).* exp(abs(x));
