%Script to optimize window of time for decoding. using firing rate in time windows
...
clear
% Select parameters
tstWin=1:20; %Array of tested time windows (from 100ms to 2000ms)
odor_to_keep = 2; %ODOR: EB = 2 (Food odor); hexa = 1 (non-Food odor)
save_flag = 1; %Flag to save (=1) or not save (=0) workspace
...
for ratNum=ind
    fileName=sprintf('Rat%d_indPC_%s_%s.mat',ratNum,odorName,drugName);
    ...
    %FirstEvok=TimeVars.FirstEvok+3; %for shifting forward by 300ms
    nPC=size(sOR,3);
    %Output (trial-avg net responses (netRespn); all LDA accuracies (OrAcc/RetAcc); averaged LDA accuracies (NeuronClass))
    RetAcc=zeros(szOptWin,nPC,numTrials);
    OrAcc=zeros(szOptWin,nPC,numTrials);
    ttlAcc=zeros(szOptWin,nPC,numTrials);
    thrsh=zeros(szOptWin,nPC,numTrials);
    switch drgState
        case 1 %No drug
            AccMat_ND=zeros(szOptWin,nPC,4);
        case 2 %Bicuculine
            AccMat_Bic=zeros(szOptWin,nPC,4);
        case 3 %Muscimol
            AccMat_Mus=zeros(szOptWin,nPC,4);
    end
    for j=1:szOptWin
        ...
        for k=1:nPC
            ...
            tmpEvkRet = sRET(FirstEvok:LastEvok,:,k); %raw evoked fRate
            tmpEvkOrth = sOR(FirstEvok:LastEvok,:,k);
            Zmn_RetTemp=sum(tmpEvkRet,1)/lenEvok;
            Zmn_OrthTemp=sum(tmpEvkOrth,1)/lenEvok;
...
if save_flag==1
    WkspcNm=sprintf('IndPCdecode_varyWin_%s_%dsSpon.mat',odorName,0);
    %WkspcNm=sprintf('IndCellRAW3shift_OptWin_%s_%dsSpon.mat',odorName,0);
